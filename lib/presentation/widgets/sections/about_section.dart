...
class AboutSection extends StatelessWidget {
    ...
    return SectionBackground(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            SectionHeader(label: 'WHO I AM', title: AppStrings.aboutTitle, subtitle: AppStrings.aboutSubtitle),
  ...
  Widget _buildMobileLayout(BuildContext context) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [_buildTextContent(context), const SizedBox(height: 48), _buildSkillsHighlight(context)],
  );
            ...
            child: Column(
              ...
              children: [
                Row(children: [
                  ...
                  Text('My Story', style: GoogleFonts.sora(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.primary, letterSpacing: 1)),
                ]),
  ...
  Widget _buildCertifications() {
    final certs = ['The Ultimate Hands-On Flutter & MVVM', 'Flutter & Dart: The Complete Guide'];
    return _GlassCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(children: [
            ...
            Text('CERTIFICATIONS', style: GoogleFonts.sora(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.primary, letterSpacing: 2)),
          ]),
          const SizedBox(height: 14),
          ...certs.map((c) => Padding(
            padding: const EdgeInsets.only(bottom: 10),
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Container(margin: const EdgeInsets.only(top: 2), child: const Icon(Icons.verified, size: 16, color: AppColors.primary)),
              const SizedBox(width: 10),
              Expanded(child: Text(c, style: GoogleFonts.dmSans(fontSize: 14, color: AppColors.textSecondary))),
            ]),
          )),
        ],
      ),
            ...
            child: Column(
              ...
              children: [
                Row(children: [
                  Text('📊', style: const TextStyle(fontSize: 18)),
                  const SizedBox(width: 8),
                  Text('EXPERTISE LEVEL', style: GoogleFonts.sora(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.primary, letterSpacing: 2)),
                ]),
                const SizedBox(height: 24),
                ...coreSkills.map((s) => Padding(
    ...
    child: Column(
      ...
      children: [
        Row(children: [
          Text('🌐', style: const TextStyle(fontSize: 18)),
          const SizedBox(width: 8),
          Text('LANGUAGES', style: GoogleFonts.sora(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.primary, letterSpacing: 2)),
        ]),
        const SizedBox(height: 14),
        Row(children: [
    ...
    return ScrollAnimatedWidget(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [BoxShadow(color: AppColors.primary.withOpacity(0.06), blurRadius: 30, offset: const Offset(0, 8))],
        ),
        child: isMobile
            ? Wrap(spacing: 20, runSpacing: 28, children: stats.map((s) => SizedBox(width: (MediaQuery.of(context).size.width - 104) / 2, child: _StatCard(stat: s))).toList())
            : Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: stats.map((s) => Expanded(child: _StatCard(stat: s))).toList()),
      ),
    );
  }
}
    ...
    child: AnimatedContainer(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        AnimatedContainer(
          ...
          decoration: BoxDecoration(color: widget.stat.color.withOpacity(_h ? 0.15 : 0.08), shape: BoxShape.circle,
            boxShadow: _h ? [BoxShadow(color: widget.stat.color.withOpacity(0.3), blurRadius: 16)] : []),
          child: Icon(widget.stat.icon, color: widget.stat.color, size: 24),
        ),
        const SizedBox(height: 12),
        Text(widget.stat.value, style: GoogleFonts.sora(fontSize: 40, fontWeight: FontWeight.w800, color: _h ? widget.stat.color : AppColors.textPrimary, letterSpacing: -1)),
        const SizedBox(height: 4),
        Text(widget.stat.label, textAlign: TextAlign.center, style: GoogleFonts.dmSans(fontSize: 13, color: AppColors.textSecondary)),
      ]),
    ),
  );
}
...
class _SkillBar extends StatefulWidget {
  final String label; final double level; final Color color;
  const _SkillBar({required this.label, required this.level, required this.color});
...
class _SkillBarState extends State<_SkillBar> with SingleTickerProviderStateMixin {
  ...
  @override void initState() {
    ...
    _a = Tween<double>(begin: 0, end: widget.level).animate(CurvedAnimation(parent: _c, curve: Curves.easeOutCubic));
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Text(widget.label, style: GoogleFonts.sora(fontSize: 14, fontWeight: FontWeight.w500, color: AppColors.textPrimary)),
        AnimatedBuilder(animation: _a, builder: (_, __) => Text('${(_a.value * 100).round()}%', style: GoogleFonts.dmMono(fontSize: 12, color: widget.color, fontWeight: FontWeight.w600))),
      ]),
      ...
      Container(height: 4, decoration: BoxDecoration(color: AppColors.border, borderRadius: BorderRadius.circular(2)),
        child: AnimatedBuilder(animation: _a, builder: (_, __) => FractionallySizedBox(
          widthFactor: _a.value, alignment: Alignment.centerLeft,
...
class _LangBadge extends StatelessWidget {
  final String lang, level; final Color color;
  const _LangBadge({required this.lang, required this.level, required this.color});
  @override
  Widget build(BuildContext context) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(lang, style: GoogleFonts.sora(fontSize: 14, fontWeight: FontWeight.w600, color: color)),
      Text(level, style: GoogleFonts.dmSans(fontSize: 12, color: AppColors.textMuted)),
    ]),
  );
}
